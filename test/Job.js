...
describe('Job', () => {
  ...
  describe('run', () => {
    ...
    beforeEach(() => {
      ...
      job = new Job({core: jobs, name: 'testRun'})
    })
    ...
    it('fails if job is undefined', done => {
      job = new Job({core: jobs, name: 'not defined'})
      job.run(() => {
  ...
  describe('touch', () => {
    it('extends the lock lifetime', done => {
      ...
      const job = new Job({core: jobs, name: 'some job', lockedAt})
  ...
  describe('save', () => {
    it('calls saveJob on the core', done => {
  ...
  describe('start/stop', () => {
    ...
    describe('events', () => {
      ...
      it('emits start event', done => {
        const job = new Job({core: jobs, name: 'jobQueueTest'})
        jobs.once('start', j => {
          expect(j).to.be(job)
          done()
        })
      ...
      it('emits fail event', done => {
        const job = new Job({core: jobs, name: 'failBoat'})
  ...
  describe('job lock', () => {
    ...
    it('does not on-the-fly lock more than core._lockLimit jobs', done => {
      jobs.lockLimit(1)
    ...
    it('does not lock more than core._lockLimit jobs during processing interval', done => {
      jobs.lockLimit(1)
  ...
  describe('job concurrency', () => {
    ...
    it('should support custom sort option', () => {
      const sort = {foo: 1}
      const core = new Core({sort})
      expect(core._sort).to.eql(sort)
    })
  })
  ...
  describe('Integration Tests', () => {
    describe('.every()', () => {
      it('Should not rerun completed jobs after restart', done => {
        ...
        const startService = () => {
          const serverPath = path.join(__dirname, 'fixtures', 'core-instance.js')
